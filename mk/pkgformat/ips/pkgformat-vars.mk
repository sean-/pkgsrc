...
PKG_FMT_CMD?=		/usr/bin/pkgfmt
PKG_MOG_CMD?=		/usr/bin/pkgmogrify
PKG_REPO_CMD?=		/usr/bin/pkgrepo
PKG_SEND_CMD?=		/usr/bin/pkgsend
...
PKG_INSTALL?=	${PKG_CMD} install -q --no-backup-be --no-refresh
PKG_ADMIN?=	${PKG_ADMIN_CMD} ${PKGTOOLS_ARGS}
#PKG_CREATE?=	${PKG_CREATE_CMD} ${PKGTOOLS_ARGS}
PKG_UNINSTALL?=	${PKG_CMD} uninstall -q
PKGFMT?=	${PKG_FMT_CMD}
PKGMOG?=	${PKG_MOG_CMD}
PKGREPO?=	${PKG_REPO_CMD}
PKGSEND?=	${PKG_SEND_CMD}
...
# package name.
#
_PKG_BEST_EXISTS?=	${PKGSRCDIR}/mk/pkgformat/ips/pkg-best-exists
_HOST_PKG_BEST_EXISTS?=	${PKGSRCDIR}/mk/pkgformat/ips/pkg-best-exists
