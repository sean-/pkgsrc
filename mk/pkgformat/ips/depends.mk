...
_REDUCE_RESOLVED_DEPENDS_CMD=${PKGSRC_SETENV} CAT=${CAT:Q}		\
			${AWK} -f ${PKGSRCDIR}/mk/pkgformat/ips/reduce-resolved-depends.awk \
				< ${_RDEPENDS_FILE}
...
_RESOLVE_DEPENDS_CMD=	\
	${PKGSRC_SETENV} _PKG_DBDIR=${_PKG_DBDIR:Q} PKG_INFO=${PKG_INFO:Q} \
		HOST_PKG_INFO=${HOST_PKG_INFO:Q} PKG_ADMIN=${PKG_ADMIN:Q} \
		_DEPENDS_FILE=${_DEPENDS_FILE:Q} \
		_HOST_PKG_BEST_EXISTS=${_HOST_PKG_BEST_EXISTS:Q} \
		_PKG_BEST_EXISTS=${_PKG_BEST_EXISTS:Q} \
...
_DEPENDS_INSTALL_CMD=							\
	...
	bootstrap|tool)							\
		...
		pkg=`${PKGSRC_SETENV} PKG_ADMIN=${PKG_ADMIN:Q} ${_HOST_PKG_BEST_EXISTS} "$$pattern" || ${TRUE}`;	\
		;;							\
	build|full)							\
		extradep=" ${PKGNAME}";					\
		cross=${USE_CROSS_COMPILE:Uno};				\
		archopt=;						\
		pkg=`${PKGSRC_SETENV} PKG_ADMIN=${PKG_ADMIN:Q} ${_PKG_BEST_EXISTS} "$$pattern" || ${TRUE}`;	\
		;;							\
	esac;								\
	case "$$pkg" in							\
	"")								\
...
pkg_install-depends:
	# We may want to implement this if pkg_admin is required long-term.
	${RUN}${DO_NADA}
